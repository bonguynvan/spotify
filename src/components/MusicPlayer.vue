  ...
  >
    <div class="flex items-center w-1/4">
      <div class="flex items-center ml-4">
        <img width="55" :src="currentArtist.albumCover" class="rounded-sm shadow-2xl" />
        <div class="ml-4">
      ...
      <div class="flex items-center ml-8">
        ...
        <PictureInPictureBottomRight fillColor="#FFF" :size="18" class="ml-4" />
      </div>
    </div>
    <div class="max-w-[35%] mx-auto w-2/4 mb-3">
      <div class="flex-cols items-center justify-center">
        <div class="buttons flex items-center justify-center h-[30px]">
          <button class="mx-2">
            <SkipBackward fillColor="#FFF" :size="25" @click="useSong.prevSong(currentTrack)" />
          </button>
          <button
            class="p-1 rounded-full mx-3 bg-white"
            @click="useSong.playOrPauseThisSong(currentArtist, currentTrack)"
          >
            <Play v-if="!isPlaying" fillColor="#000" :size="25" />
            <Pause v-else fillColor="#000" :size="25" />
          </button>
          <button class="mx-2">
            <SkipForward fillColor="#FFF" :size="25" @click="useSong.nextSong(currentTrack)" />
          </button>
        </div>
      </div>
