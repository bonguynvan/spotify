...
<template>
  <li
    class="flex items-center justify-beween rounded-md hover:bg-[#2A2929]"
  ...
  >
    <div class="flex items-center w-full py-1.5">
      <div v-if="isHover" class="w-[40px] ml-[14px] mr-[6px] cursor-pointer">
        ...
        />
        <Pause v-else fillColor="#FFF" :size="25" @click="useSong.playOrPause()" />
      </div>
      ...
      <div>
        ...
        >
          <span :class="{ 'text-green-500': currentTrack && currentTrack.name === track.name }">{{ track.name }}</span>
        </div>
