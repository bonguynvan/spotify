...
onMounted(() => {
    volume.value.addEventListener('input', function(e) {
        audio.value.volume = (e.currentTarget.value / 100)
    })
})
</script>
<template>
  <VolumeMute v-if="vol == 0" fillColor="#FFF" :size="20" @click="vol = 80" />
  <VolumeHigh v-else fillColor="#FFF" :size="20" @click="vol = 0" />
  <div
